...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @ShreedSupport - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @ShreedSupport - %(levelname)s - %(message)s",
                level=INFO)
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/ShreedUserBot/ShreedUserBot.git")

# Konsol gündeliy
CONSOLE_LOGGER_VERBOSE = sb(os.environ.get("CONSOLE_LOGGER_VERBOSE", "False"))

# SQL 
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///shreed.db")
...
if not BLACKLIST_CHAT: #Əgər ayarlanıbsa Shreed support qrupu əlavə olunur.
...
WHITELIST = get('https://raw.githubusercontent.com/shreeduserbot/shreeduserbot/main/whitelist.json').json()
...
with bot:
    if AVTO_QATILMA:
        try:
            bot(JoinChannelRequest("@ShreedSupport"))
            bot(JoinChannelRequest("@ShreedUserBot"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam mən` @ShreedUserBot`! Mən sahibimə (`@{me.username}`) kömək etmək üçün varam, yəni sənə kömək edə bilmərəm :( Amma səndə 𝚂𝙷𝚁𝙴𝙴𝙳 𝚄𝚜𝚎𝚛𝚋𝚘𝚝 qura bilərsən Dəstək qrupumuza gəl` @ShreedSupport')
            else:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@ShreedUserBot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Zəhmət olmasa sadəcə .kömek ilə kömək istəyin.",
                    text=f"**𝚂𝙷𝚁𝙴𝙴𝙳 Əla İşləyir 💥** [𝚂𝙷𝚁𝙴𝙴𝙳 𝚄𝚜𝚎𝚛𝚋𝚘𝚝](https://t.me/shreeduserbot) __İşləyir...__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
...
Hesabınızı bot'a çevirə bilərsiz və bunları işlədə bilərsiz. Unutmayın, siz başqasının botunu idarə etməssiniz! Altdakı GitHub adresində bütün qurulum detayları var""",
                    buttons=[
                        [custom.Button.url("Kanal", "https://t.me/shreeduserbot"), custom.Button.url(
                            "Qrup", "https://t.me/shreedsupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/shreed/shreeduserbot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("Hey! Mənim mesajlarımı düzəltməyə çalışma! Özünə bir @ShreedUserBot qur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**𝚂𝙷𝚁𝙴𝙴𝙳 Əla İşləyir 💥** [𝚂𝙷𝚁𝙴𝙴𝙳 𝚄𝚜𝚎𝚛𝚋𝚘𝚝](https://t.me/Shreeduserbot) __İşləyir...__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
